...
def r_squared(y, estimated):
    ...
    variance = sum((y[i] - mean)**2 for i in range(len(y)))
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        ...
        pylab.plot(x, estimated, "-r", label="best fit")
        pylab.xlabel("year")
        pylab.ylabel("temperature")
        pylab.title("Temperature data over several years with best fit degree " + str(i+1) +
                    "\n with r^2 value: " + str(r_squared(y, estimated)))
        pylab.show()
